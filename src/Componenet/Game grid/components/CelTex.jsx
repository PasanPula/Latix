...
{ 
   ...
   return(
       <div className="celtex">
           <input type="text" key={parms.id}  className={(parms.show)?corlor:'correct'} id={parms.id} value={Value} pattern="^-?[0-9]\d*(\.\d+)?$" maxLength="3" onChange={valuechnage} onMouseEnter={forcus } onFocus={()=>parms.RowColChnage(parms.id)}  onMouseLeave={yes} readOnly={(parms.show)?true:false}/>
       </div>
   );
}
